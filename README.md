...
Power Mode rules can also send a post-insertion key after successful dictation insertion: Return, Shift+Return, or Windows-adapted Ctrl+Return for macOS `commandEnter` compatibility.
...
- Power Mode section with ordered enabled/default rules, active-window quick fill from Win32 foreground-window process/title detection, session-only overrides for model/language/enhancement/prompt/cleanup settings, post-insertion auto-send keys, and history name/emoji metadata
- Microphone capture
