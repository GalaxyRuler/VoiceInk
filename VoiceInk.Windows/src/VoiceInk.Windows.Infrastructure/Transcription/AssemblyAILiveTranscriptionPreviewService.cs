...
public sealed class AssemblyAILiveTranscriptionPreviewService(
    ...
    TimeSpan? cleanupTimeout = null) : ILiveTranscriptionPreviewService
{
        ...
        CancellationToken cancellationToken)
    {
        if (!settings.ShowLiveTranscriptPreview
            ...
            || !string.Equals(
                ...
                TranscriptionProviderPresetCatalog.AssemblyAI.Id,
        ...
        try
        {
            await ConnectWithTimeoutAsync(
                    socket,
                    AssemblyAIStreamingUriFactory.Build(settings),
                    apiKey,
                    cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        return new AssemblyAILiveTranscriptionPreviewSession(socket, partialTranscriptUpdated, cleanupTimeout);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new TimeoutException("AssemblyAI live transcript preview connection timed out.");
        }
    }
    ...
    private sealed class AssemblyAILiveTranscriptionPreviewSession : ILiveTranscriptionPreviewSession
    {
        ...
        public AssemblyAILiveTranscriptionPreviewSession(
            IStreamingWebSocket socket,
            Action<string> partialTranscriptUpdated,
        ...
        public async Task CompleteAsync(CancellationToken cancellationToken)
        {
            ...
            if (sendCompleted)
            {
                await SendTerminateNoThrowAsync(cancellationToken).ConfigureAwait(false);
        ...
        private async Task ReceiveLoopAsync()
        {
            ...
            try
            {
                await foreach (var message in socket.ReceiveTextMessagesAsync(lifetime.Token)
                                   .ConfigureAwait(false))
                {
                    var transcript = AssemblyAIStreamingMessageParser.Parse(message);
        ...
        private async Task SendTerminateNoThrowAsync(CancellationToken cancellationToken)
        {
            try
            {
                using var timeout = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
                timeout.CancelAfter(cleanupTimeout);
                await socket.SendTextAsync("""{"type":"Terminate"}""", timeout.Token)
                    .WaitAsync(timeout.Token)
                    .ConfigureAwait(false);
            }
            catch
