# Windows Model Repair Action Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:test-driven-development for presenter behavior and superpowers:verification-before-completion before committing. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an actionable model health repair button.

**Architecture:** Keep model lifecycle rules in Core presenter data, and let WinUI route the selected presenter action to existing import or warmup flows.

**Tech Stack:** .NET 10, C#, WinUI 3, xUnit.

---

### Task 1: Presenter Repair Action

**Files:**
- Modify: `VoiceInk.Windows/src/VoiceInk.Windows.Core/Models/LocalWhisperModelHealthPresenter.cs`
- Modify: `VoiceInk.Windows/tests/VoiceInk.Windows.Core.Tests/Models/LocalWhisperModelHealthPresenterTests.cs`

- [x] **Step 1: Write failing presenter tests**
- [x] **Step 2: Run focused tests to verify RED**
- [x] **Step 3: Implement repair action mapping**
- [x] **Step 4: Run focused tests to verify GREEN**

### Task 2: WinUI Repair Button

**Files:**
- Modify: `VoiceInk.Windows/src/VoiceInk.Windows.App/MainWindow.xaml`
- Modify: `VoiceInk.Windows/src/VoiceInk.Windows.App/MainWindow.xaml.cs`

- [x] **Step 1: Add repair action button**
- [x] **Step 2: Route presenter action to import or warmup**
- [x] **Step 3: Run focused tests and build**

### Task 3: Docs, Review, Commit

**Files:**
- Modify: `docs/superpowers/project-completion.md`
- Modify: `docs/superpowers/specs/2026-05-24-windows-open-source-parity-design.md`

- [x] **Step 1: Update docs**
- [x] **Step 2: Run final verification**
- [x] **Step 3: Review and commit**
