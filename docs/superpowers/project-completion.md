...
```text
Windows local model filename guidance  [####################] 100%
```

Completed:

- Wrote the local model filename guidance spec and implementation plan.
- Added failing model health presenter assertions for `ggml-*.bin` guidance in ready and repair states.
- Added filename convention rows to `LocalWhisperModelHealthPresenter`.
- Ran focused model health presenter tests, full solution tests, Debug x64 build, and whitespace checking.
