...
```text
Windows settings manual backup profile row  [####################] 100%
```

Completed:

- Wrote the settings manual backup profile row spec and implementation plan.
- Added a failing Settings presenter assertion for manual Windows-profile backup export guidance.
- Added the `Windows Profile` backup guidance row to `SettingsSectionPresenter`.
