...
```text
Windows stale imported model cleanup  [####################] 100%
```

Completed:

- Added Core cleanup for imported local model references whose files are missing, invalid, empty, or suspiciously small.
- Added a `Remove Unavailable Models` AI Models action that saves settings and refreshes the imported model list.
- Cleared the selected model path only when that exact imported path was removed.
