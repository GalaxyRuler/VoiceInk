...
| AI enhancement | 84% | `[#################---]` | OpenAI-compatible enhancement, Anthropic Messages API enhancement, Local CLI hooks, presets, custom prompts, trigger-word activation, context, retries/timeouts, secure keys, toggle-enhancement shortcut, and dynamic OpenRouter/Ollama model refresh exist. Richer assistant workflows remain. |
...
```text
Windows Local CLI enhancement provider  [####################] 100%
```

Completed:

- Added the keyless Local CLI enhancement provider preset.
- Added provider-aware command-template validation for Local CLI.
- Routed Local CLI enhancement through a testable Windows process runner with VoiceInk prompt environment variables.
...
3. Continue AI enhancement parity with richer assistant workflows and prompt metadata editing.
