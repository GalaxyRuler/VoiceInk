...
```text
Windows cloud provider smoke runbook  [####################] 100%
```

Completed:

- Added a provider-by-provider user-owned-key smoke runbook.
- Covered provider metadata probe, batch recording, live preview, History metadata, backup, and key cleanup checks.
- Documented secret-handling boundaries and credential-gated failure notes.
