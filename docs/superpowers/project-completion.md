...
| AI enhancement | 80% | `[################----]` | OpenAI-compatible enhancement, Anthropic Messages API enhancement, presets, custom prompts, trigger-word activation, context, retries/timeouts, secure keys, toggle-enhancement shortcut, and dynamic OpenRouter/Ollama model refresh exist. Local CLI hooks and richer assistant workflows remain. |
...
```text
Windows Anthropic enhancement provider  [####################] 100%
```

Completed:

- Added the Anthropic provider preset with macOS-aligned Claude model choices.
- Added provider-specific Credential Manager secret naming for Anthropic.
- Routed Anthropic enhancement through the native Messages API request/response shape.
...
3. Continue AI enhancement parity with Local CLI hooks and richer assistant workflows.
