...
```text
Windows audio fallback Sound settings row  [####################] 100%
```

Completed:

- Wrote the audio fallback Sound settings row spec and implementation plan.
- Added a failing audio presenter assertion for a Windows Sound settings guidance row in the prioritized-device fallback state.
- Added the `Windows Sound Settings` health row to the all-prioritized-devices-unavailable fallback path.
- Ran focused audio presenter tests, full solution tests, Debug x64 build, and whitespace checking.
